...
export default function CourseListClient({ initialCourses }: CourseListClientProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-gray-100 p-4 flex flex-col">
      <UnderConstructionModal isOpen={showModal} onClose={() => setShowModal(false)} />
      <motion.h1
        className="text-5xl font-bold text-center mb-2 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600"
          ...
          >
            {currentCourses.map(course => (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <Card className="bg-gray-800 bg-opacity-50 hover:bg-opacity-70 transition-colors duration-300 h-full flex flex-col justify-between">
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between gap-2 text-blue-300">
                      ...
                      {Number(course.request_count) > 40 && (
